...
-- Удаление триггера для партицирования

SELECT prttn_tools.drop_ins_trigger('dbwa', 'stat_history');
...
SELECT prttn_tools.drop_ins_trigger('dbwa', 'statop_history');


-- Function: dbwa.get_stat(character varying)

-- Отмена прав на EXECUTE для всех

REVOKE ALL ON FUNCTION dbwa.get_stat(CHARACTER VARYING) FROM public;
...
-- Добавлена опция SECURITY DEFINER
